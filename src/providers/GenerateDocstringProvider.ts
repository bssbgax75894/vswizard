 ...
 */
export async function GenerateDocstring(text: string, insertionLine: number, authKey: string|undefined) {
    ...
    const startDocstringToken = workspace.getConfiguration("vswizard").get(`${langId}.startDocstringToken`) || "'''";
    const endDocstringToken = workspace.getConfiguration("vswizard").get(`${langId}.endDocstringToken`) || "'''";
    const templateNum: number = workspace.getConfiguration("vswizard").get(`${langId}.docstringTemplate`) || 0;
